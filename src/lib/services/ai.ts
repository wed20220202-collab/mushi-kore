...
export interface InsectIdentificationProvider { identify(image: ImageInput): Promise<InsectIdentificationResult>; }
...
export const IDENTIFICATION_SYSTEM_PROMPT = `あなたは日本の利用者向けの慎重な昆虫同定アシスタントです。画像を詳細に観察し、画像だけで断定せず、候補は確度順に最大3件、confidenceは0〜1で返してください。和名・英名・学名と分類は、確信がない場合に推測で細部を埋めないでください。虫ではない、または虫を確認できない場合はisInsectをfalseにし、commonNameJaは「虫を確認できませんでした」、候補は空配列にしてください。危険性や毒性は医療的・生物学的な確定診断ではなく、触れないことを安全側の既定としてください。指定JSONスキーマ以外の文章は返さないでください。`;

const IDENTIFICATION_JSON_SCHEMA = {
  type: "object",
  additionalProperties: false,
  properties: {
    isInsect: { type: "boolean" },
    commonNameJa: { type: "string", description: "日本語の一般名。確認不能なら虫を確認できませんでした" },
  ...
  required: ["isInsect", "commonNameJa", "commonNameEn", "scientificName", "order", "family", "genus", "candidates", "confidence", "appearance", "reason", "habitat", "activeSeason", "dangerLevel", "toxicity", "warnings", "uncertaintyReason"],
} as const;
...
export class GeminiInsectIdentificationProvider implements InsectIdentificationProvider {
  ...
  async identify(image: ImageInput): Promise<InsectIdentificationResult> {
    ...
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", {
      ...
      body: JSON.stringify({
        model: this.model,
        system_instruction: IDENTIFICATION_SYSTEM_PROMPT,
        input: [
          { type: "image", mime_type: image.mimeType, data: Buffer.from(image.bytes).toString("base64") },
          { type: "text", text: "この画像に写る虫を同定し、観察できる特徴と不確実性を日本語中心で返してください。" },
        ],
    ...
    return validateIdentificationResult(JSON.parse(outputText));
  }
}
...
export class MockInsectIdentificationProvider implements InsectIdentificationProvider {
  async identify(image: ImageInput): Promise<InsectIdentificationResult> {
    ...
    return validateIdentificationResult({
      isInsect: true,
      commonNameJa: "カブトムシ",
      commonNameEn: "Japanese rhinoceros beetle",
      scientificName: "Trypoxylus dichotomus",
      order: "コウチュウ目",
      family: "コガネムシ科",
      genus: "カブトムシ属",
      candidates: [
        { commonNameJa: "カブトムシ", commonNameEn: "Japanese rhinoceros beetle", scientificName: "Trypoxylus dichotomus", confidence: 0.92 },
        { commonNameJa: "コカブトムシ", commonNameEn: "Small rhinoceros beetle", scientificName: "Eophileurus chinensis", confidence: 0.05 },
        { commonNameJa: "サイカブト", commonNameEn: "Coconut rhinoceros beetle", scientificName: "Oryctes rhinoceros", confidence: 0.03 },
      ],
      confidence: 0.92,
      appearance: "大きく湾曲した頭角と、光沢のある赤褐色の上翅が見られます。",
      reason: "頭部の一本角、前胸部の形、体色がカブトムシの雄の特徴と一致します。",
      habitat: "クヌギやコナラのある雑木林、里山",
      activeSeason: "6月〜8月",
      dangerLevel: "none",
